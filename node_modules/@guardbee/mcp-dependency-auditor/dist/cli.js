...
// ── Output ─────────────────────────────────────────────────────────────────────
function printText(results, label, durationMs) {
    ...
    for (const r of vulnerable) {
        for (const v of r.vulns) {
            ...
            if (fixed)
                console.log(`  Fix     : upgrade to ${r.name}@${fixed}`);
...
function exitCode(results, failOn) {
    for (const r of results) {
        for (const v of r.vulns) {
            if (meetsThreshold(getSeverity(v), failOn))
                return 1;
        }
    }
    return 0;
...
async function runCli(cmd, rawArgs) {
    ...
    if (cmd !== "audit-pip") {
        ...
        if (deps.length > 0) {
            const pkgs = deps.map((d) => ({ name: d.name, version: d.version, ecosystem: "npm" }));
    ...
    if (cmd !== "audit-npm") {
        ...
        if (deps.length > 0) {
            const pkgs = deps.map((d) => ({ name: d.name, version: d.version, ecosystem: "PyPI" }));
            allResults.push(...(await queryOsvBatch(pkgs)));
        }
    }
    ...
    if (cfg.ignore?.length > 0) {
        for (const r of allResults) {
            r.vulns = r.vulns.filter((v) => !cfg.ignore.some((id) => v.id === id || (v.aliases ?? []).includes(id)));
        }
    }
    if (format === "json") {
