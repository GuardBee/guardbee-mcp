  ...
  for (const f of findings) {
    if (!rulesMap.has(f.patternId)) {
      rulesMap.set(f.patternId, {
        ...
        helpUri: "https://guardbee.ai/docs/prompt-injection-scanner",
        properties: {
          "problem.severity": f.severity,
          tags: ["security", "ai-security", "prompt-injection", f.category],
        },
      });
    }
    ...
    runs: [
      {
        tool: {
          driver: {
            name: "@guardbee/mcp-prompt-injection-scanner",
            version: toolVersion,
