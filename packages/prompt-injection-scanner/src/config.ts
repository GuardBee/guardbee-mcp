...
// ── guardbee.yml schema (prompt-injection-scanner section) ─────────────────────
//
...
const DEFAULTS: PromptInjectionScannerConfig = {
  failOn: "any",
  maxFiles: 5000,
  exclude: [],
};
...
export function loadConfig(
  searchDir: string,
  cliOverrides: Partial<PromptInjectionScannerConfig> = {}
): PromptInjectionScannerConfig {
  const configPath = findConfigFile(searchDir);
  if (!configPath) return { ...DEFAULTS, ...cliOverrides };

  let fileConfig: Partial<PromptInjectionScannerConfig> = {};
  try {
    const raw = readFileSync(configPath, "utf8");
    const parsed = parseYaml(raw);
    const section = parsed["prompt-injection-scanner"] as Record<string, unknown> | undefined;
    if (section) {
