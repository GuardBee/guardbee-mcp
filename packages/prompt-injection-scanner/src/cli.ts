...
function printText(findings: Finding[], scannedFiles: number, durationMs: number): void {
  if (findings.length === 0) {
    console.log(`✅ No indirect prompt injection patterns found in ${scannedFiles} file(s) (${durationMs}ms)`);
    return;
  }
  ...
  console.log(`⚠️  Found ${findings.length} possible prompt injection pattern(s) in ${scannedFiles} file(s) (${durationMs}ms)`);
...
async function runCli(rawArgs: string[]): Promise<void> {
  ...
  if (!target) {
    console.error("Usage: guardbee-prompt-injection-scanner scan <path> [--fail-on=any] [--format=text|json|sarif]");
    process.exit(2);
  }
...
function printHelp(): void {
  console.log(`@guardbee/mcp-prompt-injection-scanner

Usage (MCP server):
  guardbee-prompt-injection-scanner [serve]

Usage (CLI):
  guardbee-prompt-injection-scanner scan <path>    Scan a document/corpus for indirect prompt injection payloads
...
if (!firstArg || firstArg === "serve") {
  startServer().catch((err) => {
    process.stderr.write(`[guardbee-prompt-injection-scanner] Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
  });
