...
// ── guardbee.yml schema (agent-graph-auditor section) ───────────────────────────
//
// agent-graph-auditor:
...
const DEFAULTS: AgentGraphAuditorConfig = {
  failOn: "any",
  maxFiles: 2000,
  exclude: [],
};
...
export function loadConfig(searchDir: string, cliOverrides: Partial<AgentGraphAuditorConfig> = {}): AgentGraphAuditorConfig {
  const configPath = findConfigFile(searchDir);
  if (!configPath) return { ...DEFAULTS, ...cliOverrides };

  let fileConfig: Partial<AgentGraphAuditorConfig> = {};
  try {
    const raw = readFileSync(configPath, "utf8");
    const parsed = parseYaml(raw);
    const section = parsed["agent-graph-auditor"] as Record<string, unknown> | undefined;
    if (section) {
