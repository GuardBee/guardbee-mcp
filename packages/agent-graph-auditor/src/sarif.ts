  ...
  for (const f of findings) {
    if (!rulesMap.has(f.patternId)) {
      rulesMap.set(f.patternId, {
        ...
        helpUri: "https://guardbee.ai/docs/agent-graph-auditor",
        properties: { "problem.severity": f.severity, tags: ["security", "ai-security", "excessive-agency", f.category] },
      });
    }
    ...
    const result: SarifResult = {
      ...
      message: { text: `${f.patternName} — ${f.match} — ${f.recommendation}` },
    };
    if (f.file) {
      result.locations = [
        { physicalLocation: { artifactLocation: { uri: f.file, uriBaseId: "%SRCROOT%" }, region: { startLine: f.line, startColumn: f.column } } },
      ];
    }
    results.push(result);
  }
    ...
    runs: [
      {
        tool: {
          driver: {
            name: "@guardbee/mcp-agent-graph-auditor",
            version: toolVersion,
