...
export async function startServer() {
    ...
    server.tool("enumerate_dns", "Enumerate all DNS records for a domain and detect SPF, DMARC, DKIM misconfigurations", {
    ...
    server.tool("enumerate_subdomains", "Enumerate common subdomains for a domain and detect dangling subdomain takeover candidates", {
    ...
    server.tool("check_email_security", "Check SPF, DMARC, and DKIM configuration for a domain and get actionable remediation advice", {
        ...
        const start = Date.now();
        const result = await enumerateDomain(domain.toLowerCase().trim());
        const lines = [`Email Security Check: ${domain}`, "─".repeat(50)];
        // SPF
        lines.push("\n── SPF ─────────────────────────────────────────");
        if (!result.spf) {
            ...
            for (const f of result.spf.issues)
        ...
        if (!result.dmarc) {
            ...
            for (const f of result.dmarc.issues)
                lines.push(`  [${f.severity.toUpperCase()}] ${f.message}`);
        }
        // DKIM
        lines.push("\n── DKIM ────────────────────────────────────────");
        ...
        if (extraSelectors?.length) {
            const extraChecks = await Promise.all(extraSelectors.map(async (sel) => {
                try {
            ...
            foundDkim.push(...extraChecks.filter((s) => s !== null));
        }
    ...
    server.tool("lookup_dns", "Perform a targeted DNS lookup for specific record types on a domain", {
        ...
        try {
            ...
            switch (type) {
                ...
                case "AAAA": {
                    const recs = await dns.resolve6(domain, { ttl: true });
                    result = recs.map((r) => `${r.address}  (TTL ${r.ttl}s)`).join("\n");
                    break;
                }
                case "MX": {
