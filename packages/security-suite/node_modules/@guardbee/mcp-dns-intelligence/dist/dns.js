...
async function resolveAll(domain) {
    ...
    if (aaaa?.length) {
        records["AAAA"] = aaaa.map((r) => ({ type: "AAAA", value: r.address, ttl: r.ttl }));
    }
    if (mx?.length) {
...
// ── DMARC analysis ────────────────────────────────────────────────────────────
export function analyzeDmarc(dmarcTxt) {
    if (!dmarcTxt)
        return null;
    const raw = dmarcTxt;
    const issues = [];
    const pairs = Object.fromEntries(raw.split(";").map((s) => s.trim().split("=").map((x) => x.trim()))
        .filter((p) => p.length === 2));
...
// ── Zone transfer check ───────────────────────────────────────────────────────
async function checkZoneTransfer(domain, nameservers) {
...
export async function enumerateSubdomains(domain, concurrency = 20) {
    const results = [];
    const queue = [...COMMON_SUBDOMAINS.map((s) => `${s}.${domain}`)];
    for (let i = 0; i < queue.length; i += concurrency) {
        const batch = queue.slice(i, i + concurrency);
        const settled = await Promise.all(batch.map(async (sub) => {
            // Try A record
...
// ── Main enumeration ──────────────────────────────────────────────────────────
export async function enumerateDomain(domain) {
    const findings = [];
    ...
    const dkimChecks = await Promise.all(commonSelectors.map(async (sel) => {
...
// ── Report formatter ──────────────────────────────────────────────────────────
export function formatDnsReport(result) {
    const lines = [`DNS Intelligence Report: ${result.domain}`, "─".repeat(50)];
    // Records summary
    for (const [type, recs] of Object.entries(result.records)) {
        lines.push(`\n${type} Records:`);
        for (const r of recs)
            lines.push(`  ${r.value}${r.ttl !== undefined ? `  (TTL ${r.ttl}s)` : ""}`);
    }
    // SPF
    if (result.spf) {
        lines.push(`\nSPF: ${result.spf.raw}`);
    ...
    if (result.dmarc) {
        ...
        if (result.dmarc.rua.length)
            lines.push(`  rua: ${result.dmarc.rua.join(", ")}`);
    }
    // DKIM
...
export function formatSubdomainReport(domain, results) {
