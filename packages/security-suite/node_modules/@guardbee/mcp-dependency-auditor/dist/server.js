...
import { queryOsvBatch, getSeverity, getFixedVersion } from "./osv.js";
function formatReport(results, ecosystem, durationMs) {
    ...
    for (const result of vulnerable) {
        for (const vuln of result.vulns) {
            ...
            if (fixed)
                lines.push(`  Fix     : upgrade to ${result.name}@${fixed}`);
...
export async function startServer() {
    ...
    server.tool("audit_npm", "Audit npm dependencies from a package.json (and optionally package-lock.json) for known CVEs using the OSV database", {
        ...
        const packages = deps.map((d) => ({ name: d.name, version: d.version, ecosystem: "npm" }));
        const results = await queryOsvBatch(packages);
        const report = formatReport(results, "npm", Date.now() - start);
        return { content: [{ type: "text", text: report }] };
    });
    server.tool("audit_pip", "Audit Python dependencies from requirements.txt or pyproject.toml for known CVEs using the OSV database", {
        ...
        const packages = deps.map((d) => ({ name: d.name, version: d.version, ecosystem: "PyPI" }));
        const results = await queryOsvBatch(packages);
        const report = formatReport(results, "pip", Date.now() - start);
        return { content: [{ type: "text", text: report }] };
    });
    server.tool("audit_package", "Audit a single package by name, version, and ecosystem", {
        ...
        const start = Date.now();
        const results = await queryOsvBatch([{ name, version, ecosystem: ecosystem }]);
        const report = formatReport(results, ecosystem, Date.now() - start);
        return { content: [{ type: "text", text: report }] };
    });
    server.tool("audit_directory", "Auto-detect and audit all supported manifests (package.json, requirements.txt) in a directory", {
        ...
        if (npmDeps.length > 0) {
            if (!includeDevDeps)
                npmDeps = npmDeps.filter((d) => !d.isDev);
            const npmPkgs = npmDeps.map((d) => ({ name: d.name, version: d.version, ecosystem: "npm" }));
        ...
        if (pipDeps.length > 0) {
            const pipPkgs = pipDeps.map((d) => ({ name: d.name, version: d.version, ecosystem: "PyPI" }));
