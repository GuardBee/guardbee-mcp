...
export async function startServer() {
    ...
    server.tool("inspect_ssl", "Inspect TLS certificate, cipher suite, and protocol configuration for a single domain", {
    ...
    server.tool("inspect_ssl_bulk", "Inspect TLS configuration for multiple domains in parallel", {
        ...
        const totalIssues = results.reduce((sum, r) => sum + r.findings.filter((f) => f.code !== "OK").length, 0);
    ...
    server.tool("check_cert_expiry", "Check certificate expiry for one or more domains and highlight certificates expiring soon", {
        ...
        const lines = [`Certificate Expiry Report\n${"─".repeat(40)}`];
    ...
    server.tool("get_cert_info", "Get detailed certificate information for a domain including fingerprint, SANs, and issuer chain", {
