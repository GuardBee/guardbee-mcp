...
// ── guardbee.yml schema (tool-poisoning-scanner section) ────────────────────────
//
...
export interface ToolPoisoningScannerConfig {
  failOn: string;
  maxFiles: number;
  exclude: string[];
}

const DEFAULTS: ToolPoisoningScannerConfig = {
  failOn: "any",
  maxFiles: 5000,
  exclude: [],
};
...
export function loadConfig(searchDir: string, cliOverrides: Partial<ToolPoisoningScannerConfig> = {}): ToolPoisoningScannerConfig {
  const configPath = findConfigFile(searchDir);
  if (!configPath) return { ...DEFAULTS, ...cliOverrides };

  let fileConfig: Partial<ToolPoisoningScannerConfig> = {};
  try {
    const raw = readFileSync(configPath, "utf8");
    const parsed = parseYaml(raw);
    const section = parsed["tool-poisoning-scanner"] as Record<string, unknown> | undefined;
    if (section) {
