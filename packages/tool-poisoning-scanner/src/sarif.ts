  ...
  for (const f of findings) {
    if (!rulesMap.has(f.patternId)) {
      rulesMap.set(f.patternId, {
        ...
        helpUri: "https://guardbee.ai/docs/tool-poisoning-scanner",
        properties: { "problem.severity": f.severity, tags: ["security", "mcp-security", "ai-security", f.category] },
      });
    }
    ...
    runs: [
      {
        tool: {
          driver: {
            name: "@guardbee/mcp-tool-poisoning-scanner",
            version: toolVersion,
