...
function printText(findings: Finding[], scannedFiles: number, durationMs: number): void {
  if (findings.length === 0) {
    console.log(`✅ No tool-poisoning or confused-deputy findings in ${scannedFiles} file(s) (${durationMs}ms)`);
    return;
  }
...
async function runCli(rawArgs: string[]): Promise<void> {
  ...
  if (!target) {
    console.error("Usage: guardbee-tool-poisoning-scanner scan <path> [--fail-on=any] [--format=text|json|sarif]");
    process.exit(2);
  }
...
function printHelp(): void {
  console.log(`@guardbee/mcp-tool-poisoning-scanner

Usage (MCP server):
  guardbee-tool-poisoning-scanner [serve]

Usage (CLI):
  guardbee-tool-poisoning-scanner scan <path>   Scan MCP server source for tool poisoning / confused-deputy tools
...
if (!firstArg || firstArg === "serve") {
  startServer().catch((err) => {
    process.stderr.write(`[guardbee-tool-poisoning-scanner] Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
  });
