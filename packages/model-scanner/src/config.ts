...
// ── guardbee.yml schema (model-scanner section) ────────────────────────────────
//
// model-scanner:
...
export interface ModelScannerConfig {
  failOn: string;
  maxFiles: number;
  exclude: string[];
}

const DEFAULTS: ModelScannerConfig = {
  failOn: "any",
  maxFiles: 2000,
  exclude: [],
};
...
export function loadConfig(searchDir: string, cliOverrides: Partial<ModelScannerConfig> = {}): ModelScannerConfig {
  const configPath = findConfigFile(searchDir);
  if (!configPath) return { ...DEFAULTS, ...cliOverrides };

  let fileConfig: Partial<ModelScannerConfig> = {};
  try {
    const raw = readFileSync(configPath, "utf8");
    const parsed = parseYaml(raw);
    const section = parsed["model-scanner"] as Record<string, unknown> | undefined;
    if (section) {
