...
function printText(findings: Finding[], scannedFiles: number, durationMs: number): void {
  if (findings.length === 0) {
    console.log(`✅ No AI/LLM security issues found in ${scannedFiles} file(s) (${durationMs}ms)`);
    return;
  }
  ...
  console.log(`⚠️  Found ${findings.length} AI/LLM security issue(s) in ${scannedFiles} file(s) (${durationMs}ms)`);
...
async function runCli(rawArgs: string[]): Promise<void> {
  ...
  if (!target) {
    console.error("Usage: guardbee-ai-code-scanner scan <path> [--fail-on=any] [--format=text|json|sarif]");
    process.exit(2);
  }
...
function printHelp(): void {
  console.log(`@guardbee/mcp-ai-code-scanner

Usage (MCP server):
  guardbee-ai-code-scanner [serve]

Usage (CLI):
  guardbee-ai-code-scanner scan <path>    Scan a file or directory for insecure AI/LLM code patterns
...
if (!firstArg || firstArg === "serve") {
  startServer().catch((err) => {
    process.stderr.write(`[guardbee-ai-code-scanner] Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
  });
