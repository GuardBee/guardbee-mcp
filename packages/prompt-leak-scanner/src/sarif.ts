  ...
  for (const f of findings) {
    if (!rulesMap.has(f.patternId)) {
      rulesMap.set(f.patternId, {
        ...
        helpUri: "https://guardbee.ai/docs/prompt-leak-scanner",
        properties: { "problem.severity": f.severity, tags: ["security", "ai-security", "data-leak", f.category] },
      });
    }
    ...
    results.push({
      ...
      message: { text: `${f.patternName} (${f.maskedMatch}) — ${f.recommendation}` },
      locations: [
        {
          physicalLocation: {
    ...
    runs: [
      {
        tool: {
          driver: {
            name: "@guardbee/mcp-prompt-leak-scanner",
            version: toolVersion,
