...
function printText(findings: Finding[], scannedFiles: number, durationMs: number): void {
  if (findings.length === 0) {
    console.log(`✅ No MCP server security issues found in ${scannedFiles} file(s) (${durationMs}ms)`);
    return;
  }
  ...
  console.log(`⚠️  Found ${findings.length} MCP server security issue(s) in ${scannedFiles} file(s) (${durationMs}ms)`);
...
async function runCli(rawArgs: string[]): Promise<void> {
  ...
  if (!target) {
    console.error("Usage: guardbee-mcp-server-auditor scan <path> [--fail-on=any] [--format=text|json|sarif]");
    process.exit(2);
  }
...
function printHelp(): void {
  console.log(`@guardbee/mcp-server-auditor

Usage (MCP server):
  guardbee-mcp-server-auditor [serve]

Usage (CLI):
  guardbee-mcp-server-auditor scan <path>    Scan an MCP server's codebase for insecure tool-definition patterns
...
if (!firstArg || firstArg === "serve") {
  startServer().catch((err) => {
    process.stderr.write(`[guardbee-mcp-server-auditor] Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
  });
