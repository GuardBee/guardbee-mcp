...
// ── guardbee.yml schema (mcp-server-auditor section) ───────────────────────────
//
// mcp-server-auditor:
...
export interface McpServerAuditorConfig {
  failOn: string;
  maxFiles: number;
  exclude: string[];
}

const DEFAULTS: McpServerAuditorConfig = {
  failOn: "any",
  maxFiles: 5000,
  exclude: [],
};
...
export function loadConfig(
  searchDir: string,
  cliOverrides: Partial<McpServerAuditorConfig> = {}
): McpServerAuditorConfig {
  const configPath = findConfigFile(searchDir);
  if (!configPath) return { ...DEFAULTS, ...cliOverrides };

  let fileConfig: Partial<McpServerAuditorConfig> = {};
  try {
    const raw = readFileSync(configPath, "utf8");
    const parsed = parseYaml(raw);
    const section = parsed["mcp-server-auditor"] as Record<string, unknown> | undefined;
    if (section) {
